...
def exactVNAlg(noise_rvs, c_d_bonus, seed, args):
    ...
    # while len(L) > 1 or first_elt_depth < depth:
        ...
        for n in range(args.degree):
            ...
            if first_elt[2] == 1 and n == 0:
                VN_val = args.opt + noise_val
                state = (-1*(VN_val + c_d_bonus[child_depth]), child_depth, 1, VN_val)
            else:
                VN_val = args.opt - args.eta + noise_val
...
def main(args):
    ...
    log.info("new algorithm with value network only eta model")
