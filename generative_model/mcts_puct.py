...
class Node:
    ...
    def __init__(self, node_id, depth, opt_bool, vn_value, q_value, visit_count, true_value):
...
def mcts_benchmark(noise_rvs, seed, args):
	...
	print(root_vn_value)
	root_visit_count = 1
	root = Node(root_id, root_depth, root_opt_bool, root_vn_value, root_vn_value, root_visit_count, args.opt)
	...
	while expand_count < args.max_expansions:
		...
		while expand_current_node == 0:
			...
			parent_true_val = current_node.true_value
			...
			for d in range(args.degree):
				...
				if d == 0:
					PN_val = parent_true_val + policy_noise_val
				else:
					value_seed = np.random.seed([child_id, seed])
					value_value = np.random.uniform(-args.eta, 0)
					PN_val = parent_true_val + value_value + policy_noise_val
			...
			if child_node in node_dict:
				...
				if current_node.depth < args.depth - 1:
					...
					new_visit_count = 1
					new_opt_bool = 0
					new_true_value = parent_true_val

					if child_node == current_node.node_id*args.degree + 1:
						if current_node.opt_bool == 1:
							new_opt_bool = 1
					else:
						value_seed = np.random.seed([new_id, seed])
						value_value = np.random.uniform(-args.eta, 0)
						new_true_value = new_true_value + value_value


					noise_seed = np.random.seed([new_id, seed, 0])
					noise_val = noise_rvs[new_depth].rvs()
					new_vn_value = new_true_value + noise_val

					new_child = Node(new_id, new_depth, new_opt_bool, new_vn_value, new_vn_value, new_visit_count, new_true_value)
...
def main(args):
    ...
    log.info("mcts with value and policy network generative model")
