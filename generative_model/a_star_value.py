...
def exactVNAlg(noise_rvs, c_d_bonus, seed, args):
    ...
    #states are [-1*(UCB value) (0), depth (1), yes/no optimal path (2), value network value (3), opt_one_step (4), node_id (5), true_value (6)]
    ...
    while first_elt_depth < args.depth and count < args.max_expansions:
        ...
        for n in range(args.degree):
            ...
            noise_seed = np.random.seed([nod_id, seed])
            noise_val = noise_rvs[child_depth].rvs()

            new_true_val = first_elt[6]

            if n == 0:
                VN_val = new_true_val + noise_val
                if first_elt[2] == 1:
...
def main(args):
    ...
    log.info("new algorithm with value network only generative model")
