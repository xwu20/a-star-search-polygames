...
class Node:
	#states are [depth, yes/no optimal path, value network value, current average, number of times visited, opt_one_step, true_value]
    def __init__(self, node_id, depth, opt_bool, vn_value, q_value, visit_count, opt_one_step, true_value):
...
def mcts_benchmark(noise_rvs, seed, args):
	...
	root_visit_count = 1
	root_opt_one_step = -1
	root = Node(root_id, root_depth, root_opt_bool, root_vn_value, root_vn_value, root_visit_count, root_opt_one_step, args.opt)
	...
	while expand_count < args.max_expansions:
		...
		if current_node.depth < args.depth - 1:
			...
			new_visit_count = 1
			new_opt_bool = 0
			new_true_val = current_node.true_value
			new_vn_value = np.NINF
			new_opt_one_step = current_node.opt_one_step

			ind = randrange(args.degree - num_children)

			if current_node.node_id * args.degree + 1 in node_dict:
				value_seed = np.random.seed([new_id, seed])
				value_value = np.random.uniform(-args.eta, 0)
				new_true_val = new_true_val + value_value
			else:
				if ind == 0:
			...
			new_vn_value = new_true_val + noise_val


			new_child = Node(new_id, new_depth, new_opt_bool, new_vn_value, new_vn_value, new_visit_count, new_opt_one_step, new_true_val)
		...
		#update visitation counts and averages
		# print("path states are:")
		# print(path_states)
		# print("updating path")
		# print(new_vn_value)
...
def main(args):
    ...
    log.info("mcts with value only generative model")
